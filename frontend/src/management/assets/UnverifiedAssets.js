...
const Assets = (props) => {
    ...
    useEffect(() => {
      if(!props.fetched) {
          ...
          props.getUnverifiedAssets(token);
      }
    ...
    const changeAsset = (id, data, token) => {
        props.verifyAsset(id, data, token);
        props.getUnverifiedAssets(token);     
      }
        ...
        return (
            <React.Fragment>
                <Button
                  ...
                  onClick={() => changeAsset(rowData.id, rowData, token)}
                >
                </Button>
                <Button
                  icon="pi pi-trash"
                      ...
                      <div className="p-field p-col-12 p-md-6">
                        ...
                        <span className="p-float-label p-input-icon-right">
                            <Dropdown
                                ...
                                showClear
                                filterBy="name"
                                onChange={(e) => onInputChange(e, 'debit_account')}
...
const mapStateToProps = state =>({
    records: state.assets.unverifiedassets,
...
export default connect(
  mapStateToProps,
  {getUnverifiedAssets, verifyAsset, getAccounts} )
  (Assets);

...
import { getEbookGenres, editEbookGenre, addEbookGenre} from '../../actions/publications/publications';
...
const Genres = (props) => {
    ...
    const saveProduct = (e) => {
        ...
        if (record.name.trim()) {
            let _record = {...record};
            if (record.id) {
                props.editEbookGenre(record.id, record, token);
                ...
                props.addEbookGenre(_record, token)
                setNewRecord(_record)
                props.getEbookGenres(token);
    ...
    const handleClick = id =>{
      history.push(`/managementdashboard/ebook-genres/${id}`)
    }
...
const mapStateToProps = state =>({
    records: state.publications.ebookgenres,
...
export default connect(
  mapStateToProps,
  {getEbookGenres, editEbookGenre, addEbookGenre } )
  (Genres);

...
import { getQuarterlyUnverifiedBills, verifyQuarterlyBill } from '../../../actions/accounts/expenses';
...
const LockedSeries = (props) => {
    ...
    useEffect(() => {
      if(!props.fetched) {
          props.getQuarterlyUnverifiedBills(token);
      }
    ...
    const changeBill = (id, data, token) => {
      props.verifyQuarterlyBill(id, data, token);
      props.getQuarterlyUnverifiedBills(token);     
    }
...
const mapStateToProps = state =>({
    records: state.expenses.quarterlyunverifiedbills,
...
export default connect(
  mapStateToProps,
  {getQuarterlyUnverifiedBills, verifyQuarterlyBill},
  )
  (LockedSeries);

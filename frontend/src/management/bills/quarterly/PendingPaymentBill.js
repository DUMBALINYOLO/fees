...
import {
    getQuarterlyPendingPaymentBill,
    getBillLines
...
class Bill extends React.Component {
  ...
  componentDidMount() {
    if (this.props.token !== undefined && this.props.token !== null) {
      this.props.getQuarterlyPendingPaymentBill(this.props.match.params.id, this.props.token);
...
const mapStateToProps = state => ({
    ...
    bill: state.expenses.quarterlypendingpaymentbill,
    token: state.auth.token,
  });
  
  const AttendanceMapped = connect(
    mapStateToProps,
    {getBillLines, getQuarterlyPendingPaymentBill}
  )(Bill);

...
import { getMonthlyUnverifiedBills, verifyMonthlyBill } from '../../../actions/accounts/expenses';
...
const LockedSeries = (props) => {
    ...
    useEffect(() => {
      if(!props.fetched) {
          props.getMonthlyUnverifiedBills(token);
      }
    ...
    const changeBill = (id, data, token) => {
      props.verifyMonthlyBill(id, data, token);
      props.getMonthlyUnverifiedBills(token);     
    }
...
const mapStateToProps = state =>({
    records: state.expenses.monthlyunverifiedbills,
...
export default connect(
  mapStateToProps,
  {getMonthlyUnverifiedBills, verifyMonthlyBill},
  )
  (LockedSeries);

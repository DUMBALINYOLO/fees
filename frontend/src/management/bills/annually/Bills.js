...
import { getAnnuallyUnverifiedBills, verifyAnnuallyBill } from '../../../actions/accounts/expenses';
...
const LockedSeries = (props) => {
    ...
    useEffect(() => {
      if(!props.fetched) {
          props.getAnnuallyUnverifiedBills(token);
      }
    ...
    const changeBill = (id, data, token) => {
      props.verifyAnnuallyBill(id, data, token);
      props.getAnnuallyUnverifiedBills(token);     
    }
...
const mapStateToProps = state =>({
    records: state.expenses.annuallyunverifiedbills,
...
export default connect(
  mapStateToProps,
  {getAnnuallyUnverifiedBills, verifyAnnuallyBill},
  )
  (LockedSeries);

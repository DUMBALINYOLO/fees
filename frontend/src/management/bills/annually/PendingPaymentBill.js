...
class Bill extends React.Component {
  ...
  componentDidMount() {
    if (this.props.token !== undefined && this.props.token !== null) {
      this.props.getAnnuallyPendingPaymentBill(this.props.match.params.id, this.props.token);
...
const mapStateToProps = state => ({
    ...
    bill: state.expenses.annuallypendingpaymentbill,
    token: state.auth.token,
  });
  
  const AttendanceMapped = connect(
    mapStateToProps,
    {getBillLines, getAnnuallyPendingPaymentBill}
  )(Bill);

...
import { getDailyUnverifiedBills, verifyDailyBill } from '../../../actions/accounts/expenses';
...
const LockedSeries = (props) => {
    ...
    useEffect(() => {
      if(!props.fetched) {
          props.getDailyUnverifiedBills(token);
      }
    ...
    const changeBill = (id, data, token) => {
      props.verifyDailyBill(id, data, token);
      props.getDailyUnverifiedBills(token);     
    }
...
const mapStateToProps = state =>({
    records: state.expenses.dailyunverifiedbills,
...
export default connect(
  mapStateToProps,
  {getDailyUnverifiedBills, verifyDailyBill},
  )
  (LockedSeries);

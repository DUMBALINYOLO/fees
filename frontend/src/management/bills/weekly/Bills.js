...
import { getWeeklyUnverifiedBills, verifyWeeklyBill } from '../../../actions/accounts/expenses';
...
const LockedSeries = (props) => {
    ...
    useEffect(() => {
      if(!props.fetched) {
          props.getWeeklyUnverifiedBills(token);
      }
    ...
    const changeBill = (id, data, token) => {
      props.verifyWeeklyBill(id, data, token);
      props.getWeeklyUnverifiedBills(token);     
    }
...
const mapStateToProps = state =>({
    records: state.expenses.weeklyunverifiedbills,
...
export default connect(
  mapStateToProps,
  {getWeeklyUnverifiedBills, verifyWeeklyBill },
  )
  (LockedSeries);

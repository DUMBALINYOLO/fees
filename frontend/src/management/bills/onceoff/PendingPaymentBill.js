...
class Bill extends React.Component {
  ...
  componentDidMount() {
    if (this.props.token !== undefined && this.props.token !== null) {
      this.props.getOnceOffPendingPaymentBill(this.props.match.params.id, this.props.token);
...
const mapStateToProps = state => ({
    ...
    bill: state.expenses.onceoffpendingpaymentbill,
    token: state.auth.token,
  });
  
  const AttendanceMapped = connect(
    mapStateToProps,
    {getBillLines, getOnceOffPendingPaymentBill}
  )(Bill);

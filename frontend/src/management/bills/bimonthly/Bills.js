...
import { getBiMonthlyUnverifiedBills, verifyBiMonthlyBill } from '../../../actions/accounts/expenses';
...
const LockedSeries = (props) => {
    ...
    useEffect(() => {
      if(!props.fetched) {
          props.getBiMonthlyUnverifiedBills(token);
      }
    ...
    const changeBill = (id, data, token) => {
      props.verifyBiMonthlyBill(id, data, token);
      props.getBiMonthlyUnverifiedBills(token);     
    }
...
const mapStateToProps = state =>({
    records: state.expenses.bimonthlyunverifiedbills,
...
export default connect(
  mapStateToProps,
  {getBiMonthlyUnverifiedBills, verifyBiMonthlyBill},
  )
  (LockedSeries);

...
import {
    getBiMonthlyPendingPaymentBill,
    getBillLines
...
class Bill extends React.Component {
  ...
  componentDidMount() {
    if (this.props.token !== undefined && this.props.token !== null) {
      this.props.getBiMonthlyPendingPaymentBill(this.props.match.params.id, this.props.token);
...
const mapStateToProps = state => ({
    ...
    bill: state.expenses.bimonthlypendingpaymentbill,
    token: state.auth.token,
  });
  
  const AttendanceMapped = connect(
    mapStateToProps,
    {getBillLines, getBiMonthlyPendingPaymentBill}
  )(Bill);

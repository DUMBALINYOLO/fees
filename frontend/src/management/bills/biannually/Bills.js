...
import { getBiAnnuallyUnverifiedBills, verifyDailyBill } from '../../../actions/accounts/expenses';
...
const LockedSeries = (props) => {
    ...
    useEffect(() => {
      if(!props.fetched) {
          props.getBiAnnuallyUnverifiedBills(token);
      }
    ...
    const changeBill = (id, data, token) => {
      props.verifyBiAnnuallyBill(id, data, token);
      props.getBiAnnuallyUnverifiedBills(token);     
    }
...
const mapStateToProps = state =>({
    records: state.expenses.biannuallyunverifiedbills,
...
export default connect(
  mapStateToProps,
  {getBiAnnuallyUnverifiedBills, verifyDailyBill},
  )
  (LockedSeries);
